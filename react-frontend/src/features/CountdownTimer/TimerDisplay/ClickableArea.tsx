import useHandleCountdownState from "@features/CountdownTimer/TimerDisplay/hooks/useHandleCountdownState.tsx";

const ClickableArea = () => {
    const { handleCountdownState } = useHandleCountdownState();

    return (
        // Countdown Timer Clickable Area
        <button
            className="hover:bg-base-200 active:bg-base-100 bg-transition-all group pointer-events-auto absolute inset-5 rounded-full duration-1000 hover:duration-1000 active:duration-100"
            onClick={handleCountdownState}
        ></button>
    );
};

export default ClickableArea;
